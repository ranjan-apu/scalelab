 ...
 * JSON gets "this is not a ScaleLab design" instead of a wall of field
 * complaints about a file that was never ours to begin with.
 */
export const DESIGN_FILE_APP = 'scalelab';
 ...
 *
 * A bare `.scalelab` rather than `.scalelab.json`, following the same
 ...
 */
export const DESIGN_FILE_EXT = '.scalelab';

/** What the file picker will offer, which is broader than what we write. */
export const DESIGN_FILE_ACCEPT = '.scalelab,.scalelab.json,application/json';
...
export function parseDesignFile(text: string): DesignParseResult {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    return { ok: false, error: 'That file does not hold a ScaleLab design.' };
  }
  ...
  if (version > DESIGN_FILE_VERSION) {
    return {
      ok: false,
      error:
        'That design was saved by a newer version of ScaleLab. Reload the page and try again.',
    };
  }
  ...
  // beyond nodes, edges, annotations and the practice sheet reaches the
  // engine. The nodes are sanitized first, so a shape box out of range is
  // clamped rather than carried, and geometry stuck on a component is dropped.
  ...
  const playground = sanitizePlayground(
    (raw as { playground?: unknown }).playground,
  );

...
describe('building a backup', () => {
  it('carries the saved designs', () => {
    ...
    expect(body.data['scalelab.designs.v1']).toContain('kept');
  });
  ...
  it('omits a key that was never written rather than storing null', () => {
    ...
    const body = JSON.parse(buildBackup());
    expect(body.data).not.toHaveProperty('scalelab.layout.v1');
  });
...
describe('restoring', () => {
  it('puts the designs back', () => {
    ...
    expect(localStorage.getItem('scalelab.designs.v1')).toContain('original');
  });
  ...
  it('replaces rather than merges, and says which keys it wrote', () => {
    ...
    const r = restoreBackup(backup);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.restored).toContain('scalelab.designs.v1');
    const now = localStorage.getItem('scalelab.designs.v1')!;
  ...
  it.each([
    ...
    ['no contents', '{"app":"scalelab-backup","version":1}'],
  ...
  it('skips a session whose topology the engine could not run', () => {
    ...
    const r = restoreBackup(
      JSON.stringify({
        ...
        version: 1,
        data: {
          'scalelab.session.v1': JSON.stringify({ topology: bad }),
          'scalelab.preferences.v1': '{"tooltips":true}',
        },
      }),
    );
    expect(r.ok).toBe(true);
    expect(localStorage.getItem('scalelab.session.v1')).toBeNull();
    expect(localStorage.getItem('scalelab.preferences.v1')).not.toBeNull();
  });

  it('leaves the browser untouched when nothing in the file is readable', () => {
    saveDesign('mine', topo());
    const before = localStorage.getItem('scalelab.designs.v1');
    const r = restoreBackup(
      JSON.stringify({ app: BACKUP_APP, version: 1, data: { 'x.y': 'z' } }),
    );
    expect(r.ok).toBe(false);
    expect(localStorage.getItem('scalelab.designs.v1')).toBe(before);
  });

  it('round trips a real browser exactly', () => {
    saveDesign('one', topo());
    saveDesign('two', topo());
    localStorage.setItem('scalelab.preferences.v1', '{"tooltips":true}');

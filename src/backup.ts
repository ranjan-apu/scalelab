...
export const BACKUP_APP = 'scalelab-backup';
export const BACKUP_VERSION = 1;
export const BACKUP_EXT = '.scalelab-backup.json';
...
export function restoreBackup(text: string): BackupResult {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    return { ok: false, error: 'That file does not hold a ScaleLab backup.' };
  }
  ...
  if (version > BACKUP_VERSION) {
    return {
      ok: false,
      error: 'That backup was made by a newer version of ScaleLab.',
    };
  }

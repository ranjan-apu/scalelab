...
/* ------------------------------------------------------------------ *
 * Hello Interview Flagship 1: Ticketmaster (High-Concurrency Booking)
 * ------------------------------------------------------------------ */
...
const ticketmasterTopology: Topology = {
  ...
  annotations: [
    note(
      ...
      40,
      384,
      'Hello Interview Standard: The Virtual Waiting Room absorbs 10x traffic spikes, letting only 150 RPS reach the booking engine. Redis distributed locks prevent double-booking.',
      320,
    ),
  ],
};
...
/* ------------------------------------------------------------------ *
 * Hello Interview Flagship 2: TinyURL (URL Shortener & Fast Redirects)
 * ------------------------------------------------------------------ */
...
const tinyurlTopology: Topology = {
  ...
  annotations: [
    note(
      ...
      40,
      384,
      'Hello Interview Standard: 100:1 read-to-write ratio. With 92% CDN hit rate and 90% Redis hit rate, less than 1% of read traffic ever touches the backend database.',
      320,
    ),
  ],
};
...
/* ------------------------------------------------------------------ *
 * Hello Interview Flagship 3: LeetCode (Online Judge & Live Leaderboard)
 * ------------------------------------------------------------------ */
...
const leetcodeTopology: Topology = {
  ...
  annotations: [
    note(
      ...
      40,
      400,
      'Hello Interview Standard: Sandboxed code execution takes 180ms+, so synchronous HTTP would hang. Kafka decouples submissions, while Redis Sorted Sets provide instant O(log N) leaderboard ranking.',
      320,
    ),
  ],
};
...
export const PRESETS: Preset[] = [
  {
    ...
    description:
      'Hello Interview System Design Standard: High-concurrency flash ticket drops. A virtual waiting room throttles admissions, Redis distributed locks prevent double-booking, and circuit breakers protect payment processing.',
  ...
  },
  {
    ...
    description:
      'Hello Interview System Design Standard: Extreme read-heavy URL redirection. 92% of traffic is answered at the CDN edge and in-memory Redis cache. Base62 keys prevent collisions.',
  ...
  },
  {
    ...
    description:
      'Hello Interview System Design Standard: Asynchronous code submission judge and real-time contest leaderboard. Code runs in isolated worker pools while rankings update instantaneously via Redis Sorted Sets.',

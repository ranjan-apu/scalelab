...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  /* ---- knobs whose label names a concept, not itself ------------------ */
  {
    id: 'service-cv',
    term: 'How uneven the work is',
    short: 'Whether every request costs the same, or some cost far more',
    why: 'A component where every request takes exactly 20 ms behaves very differently from one averaging 20 ms with occasional 200 ms outliers. Unevenness alone makes queues form and pushes up the slowest requests, even with the average unchanged.',
    category: 'capacity',
    aliases: ['cv', 'coefficient of variation', 'variance', 'variability', 'uneven'],
    see: ['service-time', 'p99', 'queue-time'],
  },
  {
    id: 'headroom',
    term: 'Spare capacity',
    short: 'How many times more traffic this could take',
    why: 'The ceiling divided by what is actually arriving. Above 1x it is keeping up, below 1x it cannot, and the component with the smallest number is your bottleneck. This is the fastest way to find one without reading a chart.',
    category: 'capacity',
    aliases: ['headroom', 'spare', 'margin'],
    see: ['bottleneck', 'capacity', 'utilisation'],
  },
  {
    id: 'read-fraction',
    term: 'Read share',
    short: 'How much of the traffic reads rather than writes',
    why: 'It decides how much a replica can help. At 95% reads, adding replicas scales almost everything; at 50%, half the traffic still lands on the one primary no matter how many replicas you add.',
    category: 'capacity',
    aliases: ['read fraction', 'reads', 'read write ratio'],
    see: ['read-replica', 'database'],
  },
  {
    id: 'target-util',
    term: 'Utilisation target',
    short: 'The busyness an autoscaler tries to hold a component at',
    why: 'Set it low and you pay for idle capacity; set it high and there is no slack left to absorb a spike while new capacity boots. Around 60 to 70% is the usual compromise, and that is not a coincidence: past 70% waiting time climbs sharply.',
    category: 'capacity',
    aliases: ['target utilisation', 'setpoint', 'target'],
    see: ['autoscaler', 'utilisation', 'warmup'],
  },
  {
    id: 'partitions',
    term: 'Partitions',
    short: 'The independent lanes a log is split into',
    why: 'A consumer group takes one message per partition at a time, so partition count, not consumer count, caps how fast a group can drain. Adding consumers beyond the partition count does nothing at all.',
    category: 'capacity',
    aliases: ['partition', 'lanes'],
    see: ['streambroker', 'consumer-lag', 'shard'],
  },
  ...
  },
  {
    ...
    short: 'Writes waiting for other writes to finish',
    why: 'A database protects shared data with locks, so each write entering service waits on the writes already holding them. The wait grows with concurrent writers, not with fleet size; adding instances speeds up reads and does nothing here. When lock wait dominates, the fixes are fewer or faster writes, read replicas, or sharding the data itself.',
    category: 'capacity',
    aliases: ['lock wait', 'write lock', 'write contention'],
    see: ['database', 'read-replica', 'shard'],
  },
  {
    id: 'prefix-ceiling',
    term: 'Prefix ceiling',
    short: 'A blob store limit that applies per prefix, not store-wide',
    why: 'Object storage partitions by key prefix, and each prefix sustains only so many requests per second. A hot prefix gets refused with a slowdown while the rest of the store idles, so the fix is spreading your keys, not buying capacity. This is the S3 503 SlowDown.',
    category: 'capacity',
    aliases: ['slowdown', 'per-prefix limit', 'hot prefix'],
    see: ['objectstore', 'throttled'],
  },
  {
    id: 'rendition',
    term: 'Rendition',
    short: 'One output quality produced from one source file',
    why: 'An encode job produces the whole quality ladder: several output files per input. Storage behind the farm therefore sees the job rate multiplied by the ladder size, and that write amplification, not the upload rate, is what you size the store for.',
    category: 'throughput',
    aliases: ['quality ladder', 'bitrate ladder', 'encode output'],
    see: ['transcoder', 'objectstore'],
  },
  {
    id: 'cpu-budget',
    term: 'CPU budget',
    short: 'The hard per-request execution limit of an edge runtime',
    why: 'Edge platforms kill a request whose code runs past the budget and your traffic falls through to the origin anyway. Push heavier logic outward and the budget starts eating the very share you were trying to answer at the edge, so the measured at-edge share is the honest number, not the share your code claims.',
    category: 'capacity',
    aliases: ['cpu cap', 'cpu limit', 'execution limit'],
    see: ['edgecompute', 'edge-share'],
  },
  ...
  },
  {
    ...
    short: 'Keys on a ring, so growth moves little data',
    why: 'With plain modulo mapping, adding one node relocates nearly every key. On a ring only the neighboring range moves, so caches and stores can grow without a full migration. Hot keys still skew, since popularity is not uniform.',
    category: 'component',
    aliases: ['consistent hashing', 'hash ring'],
    see: ['partitions', 'cache'],
  },
  {
    id: 'presigned-url',
    term: 'Presigned URL',
    short: 'A temporary direct link to stored bytes',
    why: 'The server mints a short-lived URL and the client uploads or downloads straight from storage. Large bytes never pass through your app servers, which keeps bandwidth bills and latency down.',
    category: 'component',
    aliases: ['presigned url', 'signed url'],
    see: ['objectstore', 'cdn'],
  },
  {
    id: 'inverted-index',
    term: 'Inverted index',
    short: 'Words mapped to the documents holding them',
    why: 'Search flips the direction: instead of scanning documents for words, it looks up each word and intersects posting lists. That is what makes full-text queries fast, and why the index trails writes slightly.',
    category: 'component',
    aliases: ['inverted index'],
    see: ['searchindex', 'stale-search'],
  },
  {
    id: 'ttl-lock',
    term: 'TTL lock',
    short: 'A lock that expires even if the holder dies',
    why: 'Distributed locks need leases, not just ownership. If the holder crashes, the expiry frees the resource instead of freezing it forever. Pair with version checks so a stale holder cannot act late.',
    category: 'component',
    aliases: ['ttl lock', 'distributed lock', 'lease'],
    see: ['lock-contention', 'timeout'],
  },
  {
    id: 'heartbeat',
    term: 'Heartbeat',
    short: 'A periodic signal that says I am alive',
    why: 'Connections and membership both rely on them. Missed beats mark a client offline or a node suspect, while timely beats keep sockets and presence views fresh without polling.',
    category: 'component',
    aliases: ['heartbeats', 'keepalive'],
    see: ['timeout', 'crashed'],
  },
  {
    id: 'idempotency-key',
    term: 'Idempotency key',
    short: 'A key that makes a repeated request safe',
    why: 'Send a unique key with each operation and the server answers repeats from the stored result instead of doing the work twice. Retries stop being scary, which matters most where money moves.',
    category: 'component',
    aliases: ['idempotency key', 'idempotent'],
    see: ['retry', 'redelivery'],
  },
  {
    id: 'saga',
    term: 'Saga',
    short: 'A workflow with a compensation per step',
    why: 'Long workflows across services cannot hold one transaction, so each step ships with an undo that makes business sense. A failed booking cancels the hold, refunds the charge, and tells the user, in order.',
    category: 'component',
    aliases: ['sagas'],
    see: ['dead-letter', 'redelivery'],
  },
  {
    id: 'durable-execution',
    term: 'Durable execution',
    short: 'A workflow that survives crashes and waits',
    why: 'The runtime persists progress and resumes after restarts, so timers and retries outlive any single process. Use it when steps wait on humans, partners, or hours passing.',
    category: 'component',
  ...
  },
  {
    id: 'geoshard',
    term: 'Geo shard',
    short: 'Map cells that keep nearby data together',
    why: 'Partition the world into cells and nearby things land on the same shard, so proximity queries stay local. Cell size trades precision against fan-out when searching outward rings.',
    category: 'component',
    aliases: ['geo shard', 'geosharding', 'geohash'],
    see: ['shard', 'range-query'],
  },
  {
    id: 'timeseries-rollup',
    term: 'Rollup',
    short: 'Old points compacted into coarser history',
    why: 'Raw one-second points age into minute and hour aggregates, so a year of history stays cheap while recent detail stays sharp. Dashboards read rollups; only incident review touches raw points.',
    category: 'component',
    aliases: ['rollup', 'downsampling', 'downsample'],
    see: ['timeseriesdb', 'retention'],
  },
  {
    id: 'sketch-counter',
    term: 'Sketch',
    short: 'Tiny approximate counts for huge streams',
    why: 'Probabilistic structures answer membership, cardinality, and frequency in kilobytes where exact tallies need gigabytes. Pick them when close enough drives the call, and know the error bound.',
    category: 'component',
    aliases: ['sketch', 'hyperloglog', 'bloom filter', 'count-min'],
    see: ['throughput', 'cache'],
  },
  {
    id: 'vector-ann',
    term: 'Vector ANN search',
    short: 'Nearest-neighbor search over embeddings',
    why: 'Items become points in embedding space and queries ask for the closest ones, which powers semantic search and recommendations. Indexes rebuild in batches, so freshness always lags writes.',
    category: 'component',
    aliases: ['vector ann', 'ann search', 'embeddings'],
    see: ['vectordb', 'stale-search'],
  },
  {
    id: 'cdc',
    term: 'CDC',
    short: 'Followers tailing the database commit log',
    why: 'Change data capture streams commits to search indexes, caches, and warehouses instead of polling or dual-writing. Lag becomes explicit and measurable, and any follower can rebuild by replaying.',
    category: 'component',
    aliases: ['change data capture', 'commit log'],
    see: ['replication-lag', 'consumer-lag'],
  },
];

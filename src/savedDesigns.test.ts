...
describe('saving', () => {
  ...
  it('lists the newest first', () => {
    ...
    const raw = JSON.parse(localStorage.getItem('scalelab.designs.v1')!);
    raw[0].savedAt -= 10_000;
    localStorage.setItem('scalelab.designs.v1', JSON.stringify(raw));
...
describe('reading a shelf that cannot be trusted', () => {
  it.each([
    ...
    localStorage.setItem('scalelab.designs.v1', stored);

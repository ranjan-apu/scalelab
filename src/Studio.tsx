...
/* The studio's design system and faces, loaded with the studio rather than
   at the root: the landing paints from its own inline styles in index.html
   and must not wait on 2,300 lines of workspace CSS. Vite ships these with
   the studio's dynamic-import chunk and waits for them before it runs. */
...
import { buildShareUrl, decodeTopology, shareName, sharePayload } from './share';
...
function StudioView() {
  ...
  const advisor = usePreference('advisor');
  const hldRfc = usePreference('hldRfc');
  useEffect(() => {
  ...
  const menuItems = useMemo(
    ...
    [handleNewCanvas, openGlossary, coarse, handleExportHldMarkdown, costEstimator, advisor, hldRfc],
  );
...
/**
 * The studio workspace with auth attached.
 *
 * Auth lives here rather than at the app root so the landing page boots
 * without it: a visitor reading the landing should not trigger a session
 * request they will never use. The moment the studio mounts, the provider
 * starts and the header's LoginButton sees real state.
 */
